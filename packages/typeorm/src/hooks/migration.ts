...
import { TimescaleDB, generateTimestamptzCheck } from '@sivium/timescale-db-core';
...
import { AggregateColumnOptions, AggregateType } from '@sivium/timescale-db-schemas';
...
const shouldSkipTimescale = (ds: DataSource): boolean => {
  if (!ds || !ds.options) return true;

  const name = ds.options.name ?? '';

  // Adjust rule: contains "timescale"
  return !name.toLowerCase().includes('timescale');
};

DataSource.prototype.initialize = async function () {
  if (shouldSkipTimescale(this)) {
...
DataSource.prototype.runMigrations = async function (options?: { transaction?: 'all' | 'none' | 'each' }) {
  if (shouldSkipTimescale(this)) {
    return originalRunMigrations.call(this, options);
  }
...
DataSource.prototype.synchronize = async function (dropBeforeSync: boolean = false) {
  if (shouldSkipTimescale(this)) {
    return originalSynchronize.call(this, dropBeforeSync);
  }
  if (dropBeforeSync) {
    await removeTimescaleObjects(this);
  }
